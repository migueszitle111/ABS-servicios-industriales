...
function PortfolioCard({ item, index, mobile = false, compact = false }) {
	...
	return (
		<motion.div
			...
			/>
			<div
				className={`absolute inset-0 z-30 flex flex-col items-center justify-center px-6 text-center transition-all duration-500 ease lg:px-8 opacity-0 lg:group-hover/tes:opacity-100 ${
					showOverlay ? "opacity-100" : ""
				}`}>
				<h1
					className="max-w-[90%] hyphens-auto break-words rounded-md bg-[color:rgba(5,5,5,0.65)] px-3 py-2 text-[1.35rem] font-bold leading-[1.15] text-white shadow-lg sm:text-[1.55rem] lg:max-w-none lg:bg-transparent lg:px-0 lg:py-0 lg:text-[2.05rem] lg:shadow-none"
					style={{ textShadow: "0 2px 8px rgba(0,0,0,0.85), 0 0 4px rgba(0,0,0,0.7)", overflowWrap: "break-word", wordBreak: "break-word" }}>
					{item.title}
				</h1>
			</div>
		</motion.div>
